...
CREATE TABLE proizvodi (
    id INT PRIMARY KEY,
    naziv_proizvoda VARCHAR(50) UNIQUE NOT NULL,
    datum_unosa DATE DEFAULT SYSDATE NOT NULL,
    kolicina INT DEFAULT 0,
    masa VARCHAR(5)
);

ALTER TABLE sumirano ADD aktivan CHAR(2) DEFAULT 'da';
INSERT INTO sumirano (kljuc, vrijednost, aktivan) VALUES ('broj_zaposlenika', '124', 'da');

ALTER TABLE sumirano
    MODIFY vrijednost VARCHAR(1024) DEFAULT 'nema vrijednosti';
    
ALTER TABLE profesori DROP COLUMN adresa;

TRUNCATE TABLE sumirano;

SELECT
    ime || ' ' || prezime AS puno_ime,
    LENGTH(ime) AS duljina_imena,
    LENGTH(prezime) AS duljina_prezimena,
    SUBSTR(ime, 1, 1) || '.' || SUBSTR(prezime, 1, 1) || '.' AS inicijali
FROM studenti;

SELECT *
FROM studenti
WHERE jmbag LIKE '811%'
...
FROM studenti
WHERE
    SUBSTR(jmbag, 1, 3) = '811'
ORDER BY prezime ASC, ime DESC;

SELECT *
FROM studenti
WHERE
    EXTRACT(MONTH FROM datum_rodenja) = 1
ORDER BY datum_rodenja DESC;

SELECT *
FROM smjerovi
WHERE
    otvoren_od >= 2010 AND otvoren_od <= 2015
ORDER BY aktivan ASC;
...
UPDATE statusi
SET naziv_statusa = 'Aktivan'
WHERE naziv_statusa = 'Aktivni student/ica';
...
DELETE FROM ocjene
WHERE EXTRACT(YEAR FROM datum_ocjene) = 2010;
...
FROM studenti
    INNER JOIN statusi
        ON studenti.status_id = statusi.id
WHERE
    naziv_statusa = 'Zamrznuta godina'
    OR
    naziv_statusa = 'Isklju?en';
    
SELECT
    ocjena,
    ime || ' ' || prezime AS student,
    naziv_predmeta,
    EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM datum_ocjene) AS prije_godina
FROM ocjene
    INNER JOIN studenti
